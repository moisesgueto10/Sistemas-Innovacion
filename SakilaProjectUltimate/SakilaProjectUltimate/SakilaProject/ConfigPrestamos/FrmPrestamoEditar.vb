...
Public Class FrmPrestamoEditar
    ...
    Private codigoisbnoriginal As String = ""

    Private Sub FrmPrestamoEditar_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        cargarDatos()
        ...
        codigoisbnoriginal = cbISBN.Text
    End Sub
    ...
    Private Sub BtnCancelar_Click(sender As Object, e As EventArgs) Handles BtnCancelar.Click
        frmMenu.AbrirFormEnPanel(Of GestionPrestamos)()
        Me.Dispose()
    End Sub
    ...
    Private Sub BtnGuardarNuevo_Click(sender As Object, e As EventArgs) Handles BtnGuardarNuevo.Click
        Dim id_ejemplar As Integer
        Dim comando As String = "", registros As MySqlDataReader = Nothing
        If Trim(txtidentidad.Text) <> "" And Trim(txtentrega.Text) <> "" And Trim(txtprestamo.Text) <> "" And Trim(cbISBN.Text) <> "" And IsNumeric(txtidentidad.Text) And Trim(CBRecibido.Text) <> "" And Trim(CBRetraso.Text) <> "" Then
            Dim sql1 As String = "select * from ejemplar WHERE CodigoISBN='" & cbISBN.Text & "';"
            ...
            If cbdevolucion.Text = "" Then
                comando = "UPDATE `biblioteca`.`prestamo_libro` SET `id_ejemplar`='" & id_ejemplar & "', `identidad`='" & txtidentidad.Text & "', `fecha_prestamo`='" & Convert.ToDateTime(txtprestamo.Text).ToString("yyyy-MM-dd hh:mm:ss") & "', `fecha_entrega`='" & Convert.ToDateTime(txtentrega.Text).ToString("yyyy-MM-dd hh:mm:ss") & "', `recibido`='" & CBRecibido.Text & "', `retraso`='" & CBRetraso.Text & "' WHERE `id_prestamo_libro`='" & misfunciones.getId & "';"
            Else
                comando = "UPDATE `biblioteca`.`prestamo_libro` SET `id_ejemplar`='" & id_ejemplar & "', `identidad`='" & txtidentidad.Text & "', `fecha_prestamo`='" & Convert.ToDateTime(txtprestamo.Text).ToString("yyyy-MM-dd hh:mm:ss") & "', `fecha_entrega`='" & Convert.ToDateTime(txtentrega.Text).ToString("yyyy-MM-dd hh:mm:ss") & "', `fecha_devolucion`='" & Convert.ToDateTime(cbdevolucion.Text).ToString("yyyy-MM-dd hh:mm:ss") & "', `recibido`='" & CBRecibido.Text & "', `retraso`='" & CBRetraso.Text & "' WHERE `id_prestamo_libro`='" & misfunciones.getId & "';"
            End If
            ...
            ElseIf CBRecibido.SelectedIndex = 0 Then
                comando = "UPDATE `biblioteca`.`ejemplar` SET `Disponibilidad`='Disponible' WHERE `CodigoISBN`='" & cbISBN.Text & "';"
            End If
            mibd.sentencias(comando)
            If codigoisbnoriginal <> cbISBN.Text Then
                comando = "UPDATE `biblioteca`.`ejemplar` SET `Disponibilidad`='Disponible' WHERE `CodigoISBN`='" & codigoisbnoriginal & "';"
                mibd.sentencias(comando)
            End If
            Me.Dispose()
        Else
    ...
    Private Sub cbISBN_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbISBN.SelectedIndexChanged
    ...
    Private Sub cargarejemplar()
        ...
        sql = "select * from ejemplar WHERE (id_libro='" & CBLibro.SelectedItem.value & "' AND Disponibilidad='Disponible' AND id_campus='" & misfunciones.getcampus & "') OR (CodigoISBN='" & cbISBN.Text & "' AND id_libro='" & CBLibro.SelectedItem.value & "');"
...
#Region "EVENTO DE LOS CALENDARIOS"
    ...
    Private Sub MonthCalendar1_DateChanged_1(sender As Object, e As DateRangeEventArgs) Handles Calendario3.DateChanged
    ...
    Private Sub Calendario2_DateChanged(sender As Object, e As DateRangeEventArgs) Handles Calendario2.DateChanged
        ...
        If txtprestamo.Text <> "" And txtentrega.Text <> "" Then
            If (DateTime.Compare(Convert.ToDateTime(txtprestamo.Text).ToString("yyyy-MM-dd HH:MM:ss"), Convert.ToDateTime(txtentrega.Text).ToString("yyyy-MM-dd HH:MM:ss")) < 0) Then
