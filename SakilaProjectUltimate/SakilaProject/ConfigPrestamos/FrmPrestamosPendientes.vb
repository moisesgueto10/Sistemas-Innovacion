...
Public Class FrmPrestamosPendientes
    Private idprestamo, idejemplar, ididentidad As Integer
    Private idlibro As Integer
    Private datos As MySqlDataReader

    Private Sub FrmPrestamosPendientes_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Actualizar()
    End Sub
    ...
    Sub cargarcantidad()
        ...
        If misfunciones.getIdusuario <> "root" Then
            sq1l = "SELECT COUNT(*) as conta FROM biblioteca.prestamo_libro INNER JOIN ejemplar ON ejemplar.id_ejemplar=prestamo_libro.id_ejemplar WHERE recibido='NO' AND id_campus='" & misfunciones.getcampus & "';"
        Else
            sq1l = "SELECT COUNT(*) as conta FROM biblioteca.prestamo_libro WHERE recibido='NO';"
        End If
    ...
    Private Sub cargarLVDatos()
        ...
        Dim query As String = ""
        If misfunciones.getIdusuario <> "root" Then
            query = "SELECT * FROM `gestion prestamos` WHERE recibido1='NO' AND campus='" & misfunciones.getcampus & "';" 'VISTAS
        Else
            query = "SELECT * FROM `gestion prestamos` WHERE recibido1='NO';"
        End If
    ...
    Private Sub lvPrincipal_SelectedIndexChanged_1(sender As Object, e As EventArgs) Handles lvPrincipal.SelectedIndexChanged
        Try
    ...
    Private Sub lvPrincipal_SelectedIndexChanged(sender As Object, e As EventArgs)
        Try
            idprestamo = Val(lvPrincipal.SelectedItems(0).SubItems(1).Text)
            misfunciones.setId(idprestamo)
            ididentidad = Val(lvPrincipal.SelectedItems(0).SubItems(3).Text)
