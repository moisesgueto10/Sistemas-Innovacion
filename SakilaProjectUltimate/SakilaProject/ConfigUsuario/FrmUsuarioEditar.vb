        ...
        If Trim(TxtNombre.Text) <> "" And Trim(TxtApellido.Text) <> "" And Trim(TxtEdad.Text) <> "" And Trim(TxtDireccion.Text) <> "" Then
            If cambioscarrera = 1 And cambioscampus = 0 Then
                sql = "Update `biblioteca`.`usuario` Set `id_carrera` = '" & Cbcarrera.SelectedItem.value & "', `id_campus` = '" & misfunciones.getId2 &
                               ...
                               "', `fecha_de_nacimiento` = '" & txtedad.Text & "', `Direccion` = '" & TxtDireccion.Text & "', `Num_Identidad` = '" & TxtIdentidad.Text & "' WHERE `id_usuario`='" & misfunciones.getId & "';"
                mibd.sentencias(sql)
            ...
            ElseIf cambioscampus = 0 And cambioscarrera = 0 Then
                sql = "Update `biblioteca`.`usuario` Set `id_carrera` = '" & misfunciones.getId1 & "', `id_campus` = '" & misfunciones.getId2 & "', `Nombre` = '" & TxtNombre.Text & "', `Apellido` = '" & TxtApellido.Text & "', `Telefono` = '" & TxtTelefono.Text & "', `fecha_de_nacimiento` = '" & txtedad.Text & "', `Direccion` = '" & TxtDireccion.Text & "', `Num_Identidad` = '" & TxtIdentidad.Text & "' WHERE `id_usuario`='" & misfunciones.getId & "';"
                mibd.sentencias(sql)
            End If
            frmMenu.AbrirFormEnPanel(Of GestionUsuarios)()
